...
describe('Tuppers domain helpers', () => {
  ...
  it('keeps Tuppers routes, UI, repository and rules wired', () => {
    ...
    assert.match(app, /data-dialog-title/);
    assert.match(app, /data-submit-label/);
    assert.match(app, /tt\('unassign'\)/);
    assert.match(app, /tt\('edit'\)/);
    assert.match(app, /tt\('updated'\)/);
    ...
    assert.match(script, /updateTupper/);
    assert.match(script, /openEditForm/);
    ...
    assert.match(repository, /export async function updateTupper/);
    assert.match(repository, /syncAssignedTupperName/);
    assert.match(repository, /previousLabel = `Tupper: \$\{tupper\.name\}`/);
